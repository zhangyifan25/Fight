...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            ...
            x, weights = blk(x,)
            embeds.append(x)
            attns.append(weights) #ori
            # for i in range(0,101):
            #     for j in range(0,101):
            #         cos_sim_ith_list.append(torch.nn.functional.cosine_similarity(x[:, i, :], x[:, j, :], dim=1, eps=1e-08).mean().cpu())
            # cos_sim_ith = np.mean(cos_sim_ith_list)
            # cos_sim.append(cos_sim_ith)
            # attns.append(weights[:,:,1:,1:])
        x = self.norm(x)
        embeds[-1] = x
        return x[:, 0], x[:, 1:], embeds[self.aux_layer][:, 1:] # ori
        # return x[:, 0], x[:, 1:], embeds[self.aux_layer][:, 1:], attns
