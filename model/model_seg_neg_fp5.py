...
class network(nn.Module):
    def __init__(self, backbone, num_classes=None, pretrained=None, init_momentum=None, aux_layer=None):
        ...
        self.classifier = nn.Conv2d(in_channels=self.in_channels[-1], out_channels=self.num_classes-1, kernel_size=1,  #分类头要去除背景
                                    bias=False, )
        self.aux_classifier = nn.Conv2d(in_channels=self.in_channels[-1], out_channels=self.num_classes-1,  #分类头要去除背景
    ...
    def forward(self, x, cam_only=False, crops=None, n_iter=None):
        ...
        if crops is None:
            ...
            #return cls_x4, seg, foreground_seg, _x4, cls_aux,
        else:
