...
def cam_to_label(cam, cls_label, img_box=None, bkg_thre=None, high_thre=None, low_thre=None, ignore_mid=False, ignore_index=None):
    ...
    #_pseudo_label[cam_value<=bkg_thre] = ignore_index

    if img_box is None:
        return _pseudo_label

    if ignore_mid:
        _pseudo_label[cam_value<=low_thre] = ignore_index
...
def cam_to_label_ignore(cam, cls_label, img_box=None, bkg_thre=None, high_thre=None, low_thre=None, ignore_mid=False, ignore_index=None):
    b, c, h, w = cam.shape
    #pseudo_label = torch.zeros((b,h,w))
    cls_label_rep = cls_label.unsqueeze(-1).unsqueeze(-1).repeat([1,1,h,w])
    valid_cam = cls_label_rep * cam
...
def dino_mask(cam_label,dino_map,ignore_index=255):
    ...
    #res=(res*n_iter+a)/(n_iter+1)
    #print(res)

    aff_label = (_cam_label_rep == _cam_label_rep_t).type(torch.long)
    aff_label = (dino_refine==1).type(torch.long)
...
def refine_cams_with_bkg_v2(ref_mod=None, images=None, cams=None, cls_labels=None, high_thre=None, low_thre=None, ignore_index=False,  img_box=None, down_scale=2):
    ...
    cls_labels = torch.cat((bkg_cls, cls_labels), dim=1)#[b,21]相当于0类背景均有

    refined_label = torch.ones(size=(b, h, w)) * ignore_index+1
    refined_label = refined_label.to(cams.device)
    refined_label_h = refined_label.clone()
    refined_label_l = refined_label.clone()
    
    cams_with_bkg_h = torch.cat((bkg_h, cams), dim=1)#[2,21,448,448]
    ...
    for idx, coord in enumerate(img_box):
        ...
        valid_key = torch.nonzero(cls_labels[idx,...])[:,0]#[0,duiyingclass...]
    ...
    refined_label=refined_label-1
    refined_label[refined_label_h == 0] = ignore_index
    #refined_label[(refined_label_h + refined_label_l) == 0] = ignore_index
